...
import datetime
import os
# RAJA_BHAI JOIN TO MORE UPDATES 
# Insert your Telegram bot token here
bot = telebot.TeleBot('7550636624:AAHw612XCydGXKYSpXavCsLUBA0tZsLPX7U')
# DEVELOPER --> @FATHEROFDEVELOPERDEV
# Admin user IDs
admin_id = ["7855020275"]
...
def show_plan(message):
   ...
   # response += "- Pro Plan: $20/month\n"
    ...
    response = "- 𝐃𝐌 𝐌𝐄 -- @FATHEROFDEVELOPERDEV\n"
...
def show_user_id(message):
    user_id = str(message.chat.id)
    response = f"𝐂A𝐍𝐀𝐑𝐘 𝐀𝐏𝐊 --> https://t.me/c/2312148209/303"
    bot.reply_to(message, response, parse_mode='Markdown')

@bot.message_handler(commands=['RAJA'])
def show_help(message):
    response = """𝐈 𝐊𝐍𝐎𝐖 𝐘𝐎𝐔 𝐂𝐎𝐌𝐌𝐀𝐍𝐃 𝐈𝐒 --> 𝐒𝟒 \n𝐁𝐔𝐓 𝐇𝐈𝐒 𝐁𝐎𝐓𝐒 𝐅𝐀𝐓𝐇𝐄𝐑 @FATHEROFDEVELOPERDEV \n𝐎𝐖𝐍𝐄𝐑 𝐎𝐅 @FATHEROFDEVELOPERDEV
"""
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.row(
